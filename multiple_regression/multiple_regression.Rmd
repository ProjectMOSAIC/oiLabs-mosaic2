...
Th study suggests a fundamental phenomenon of better looking teachers receiving
...
To fix this problem, we can add random noise to the points by using the function `jitter()`. 
...
6.  Use residual plots to evaluate whether the conditions of least squares
    ...
    We have included code from the Open Intro Simple Regression Lab to serve as 
    a template for your work (hint: change the variable and data names before running)
...
Either the histogram with the Normal distribution overlay or the QQ-plot 
can be used to assess the conditions of least squares regression. 
...
```{r bty-rel}
gf_point(bty_avg ~ bty_f1lower, data = evals, xlab = "Beauty Rating of Professor from First or Second Year Female", ylab = "Avg Beauty Rating")
...
m_bty_gen <- lm(score ~ bty_avg + gender, data = evals)
l <- makeFun(m_bty_gen)
gf_point(score ~ bty_avg, data = m_bty_gen, color = ~ gender, xlab = "Avg Beauty Score", ylab = "Avg Professor Evaluation Score") %>% 
  gf_fun(l(bty_avg, gender = "male") ~ bty_avg, color = ~"male") %>%
  gf_fun(l(bty_avg, gender = "female") ~ bty_avg, color = ~"female")
```
