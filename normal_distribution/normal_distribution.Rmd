...
```{r load-data, message=FALSE, warning=FALSE, results='hide', eval=TRUE}
library(mosaic)
library(oilabs)
head(bdims)
```
...
Next we want to make a density histogram to use as the backdrop and use the `fit` argument to overlay a normal probability curve. 
XX edit this for ggformula 
The `gf_hist()` command has a parameter called `type` that allows us to specify if we want to see `count`, `percent` or `density` in our histogram. Setting `type="count"` produces a frequency histogram, which shows the number of data points that fall into a particular bin. In a frequency histogram, the *heights* of the bars add up to the total number of observations. 

```{r mosaic-hist1}
histogram(~ hgt, data = fdims, type = "count")
gf_histogram( ~ hgt, data = fdims)
```
...
XX NH ggformula doesn't seem to have this option; should we consider adding this function to the ggformula package? 

```{r mosaic-hist2}
histogram(~ hgt, data = fdims, type = "percent")
gf_percents( ~ hgt, data = fdims)
```
...
```{r, echo=FALSE, fig.width=10, eval=TRUE, message=FALSE, results='hide'}
