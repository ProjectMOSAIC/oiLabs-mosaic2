...
```{r, echo=FALSE, fig.width=10, eval=FALSE, message=FALSE, results='hide'}
...
qqnormsim2 <- function (sample, data, reps = 8) 
{
    orig.data <- eval(substitute(sample), data)
    PlotData <-
      expand.grid(sample = 1:(reps + 1), row = 1:nrow(data)) %>%
      mutate(x = rnorm((reps + 1) * nrow(data), mean = mean(orig.data), sd = sd(orig.data))) 
    
    orig <- base::sample(1:(reps + 1), 1)
    
    PlotData[PlotData$sample == orig, "x"] <- orig.data
      
    ggplot(data = PlotData, aes_string(sample = "x")) + 
      stat_qq() + 
