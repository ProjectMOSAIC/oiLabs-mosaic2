...
If we wish to plot `line`s instead of `point`s, we can change the `plottype` in the {ggformula} function that you type. `plottype` describes the type of plot desired (points, lines, a histogram, etc.)
...
You might wonder how you are supposed to know that it was possible to change the plot to represent the x, y pairs as lines, rather than circles. When you type the following code: 
...
This lab was adapted for OpenIntro by Andrew Bray and Mine &Ccedil;etinkaya-Rundel from a lab written by Mark Hansen of UCLA Statistics. Additionally, this lab was adapted using functions in the `mosaic` and `ggformula` packages by Nicholas Horton and Bonnie Lin of Amherst College Statistics.
</div>
