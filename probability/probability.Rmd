...
The custom function `calc_streak()`, which was loaded in with the `oilabs` package, may be 
...
one slip says `heads` and the other says `tails`. The function `resample()` draws 
...
governs how many samples to draw (since we are `resample`-ing, we put
...
```{r sim-fair-coin}
flips <- 100
sim_fair_coin <- do(flips) * resample(outcomes, size = 1)
```

To view the results of this simulation, use the `tally()` function to count 
up the number of heads and tails.
...
sim_unfair_coin <- do(flips) * resample(outcomes, size = 1, prob = c(0.2, 0.8))
tally(~ sim_unfair_coin)
```
...
Another way of thinking about this is to think of the outcome space as a bag of 
10 chips, where 2 chips are labeled "head" and 8 chips "tail". Therefore at each
draw, the probability of drawing a chip that says "head"" is 20%, and "tail" is 80%.

If you want to learn more about `resample()` or any other function, recall that you 
can always check out its help file.

```{r help-resample,tidy = FALSE}
?resample
```
...
4.  What change needs to be made to the `resample()` function so that it reflects a 
