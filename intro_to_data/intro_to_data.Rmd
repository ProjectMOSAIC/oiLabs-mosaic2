...
Let's start with some histograms. We can create the histogram for the age of
our respondents with the following command:

```{r hist-age-geom}
gf_histogram(~ age, data = cdc)
```
...
XX BL note that this is a mess.  We need to talk.  

```{r hist-bmi}
gf_histogram(~ age, data = cdc, geom = "histogram", binwidth = 1)
...
sample frequencies.  The function `table` does this for you by counting the number 
of times each kind of response was given. For example, to see the number of 
people who have smoked 100 cigarettes in their lifetime, type
>>>>>>> mydplyr/master
