...
Some people define statistics as the field that focuses on turning information into
...
```{r load-data}
cdc <- read_csv("https://www.openintro.org/stat/data/cdc.csv")
```
By default, `read_csv()` prints the variable names. These messages can be 
suppressed using the `message = FALSE` code chunk option to save space and 
improve readability.
...
summary: minimum, first quartile, median, third quartile, maximum, mean, standard 
deviation, number of observations, and number of missing values.
For `weight`, this is:
...
This takes the `cdc` dataset and **pipes** it into the `select` function. This 
function then select the variable called `weight`, and **pipe** this 
...
You can use the `favstats()` function in the `mosaic` package to collect summary
statistics:
...
The output of the `favstats()` function summarizes several descriptive statistics that 
are useful for numerical data. If we are interested in finding specific statistics, some 
useful function calls for a single numerical variable are as follows:
...
```
The output of this function is a contigency table. Here, we see column labels of 0 and 1. 
Recall that 1 indicates a respondent has smoked at least 100 cigarettes. The rows 
refer to gender. To create a mosaic plot (no relation to the package name) of this table, 
we would enter the following command: 

```{r mosaic-smoke-gender}
tally(gender ~ smoke100, data = cdc) %>% 
  vcd::mosaic(shade = TRUE, legend = F)

#XX NH 
# Colors are misleading; do we want this? 
```
...
*Environment* tab, but we don't want to print the entire dataset since it's pretty large.
Taking a look at just the first few lines should give you a good idea of what it
looks like. By typing the following command, we can view the first few 
...
5.  Create a new object called `under23_and_smoke` that contains all observations 
...
```{r height-weight}
gf_point(height ~ weight, data = cdc, geom = "point")
```
...
```{r barplot-smoke-gender}
gf_bar(~ gender, data = cdc)
...
gf_boxplot(height ~ gender, data = cdc)
```

We have box plots of the heights where the groups are defined by gender. 
...
cdc <- 
  cdc %>%
  mutate(bmi = (weight / height^2) * 703)
gf_boxplot(bmi ~ genhlth, data = cdc)
```
...
```{r boxplot-bmi2}
gf_boxplot(bmi ~ as.factor(smoke100), data = cdc)
```
...
Finally, let's make some histograms. We can create the histogram for the age of
our respondents with the following command:

```{r hist-age}
gf_dhistogram(~ age, data = cdc)
```
...
```{r hist-bmi}
gf_dhistogram(~ bmi, data = cdc)
gf_dhistogram(~ bmi, data = cdc, bins = 50)
```
