...
In this lab we will explore the data using the `mosaic` package and visualize it 
using the `ggformula` package, which is found within the `mosaic` package for data
visualization. The data can be found in the companion package for OpenIntro 
labs, `oilabs`.
...
This returns the names of hte variables in this data frame, like `year`, `month`, `day`,
`dep_time`, `dep_delay`, `arr_time`, `arr_delay`, `carrier`, etc. The **codebook**
...
the console, but that might be unwise here.  We know `nycflights` has 32,735 rows, so 
...
```{r hist-dep-delay}
gf_histogram(~ dep_delay, data = nycflights) %>%
  gf_labs(x = "Departure Delays, in minutes", y = "Count")
```

The `gf_histogram()` function, from the `ggformula` package called by the `mosaic`
package, plots the `dep_delay` variable from the `nycflights` data 
frame on the x-axis. 
...
gf_histogram(~ dep_delay, data = nycflights, binwidth = 15) %>%
  gf_labs(x = "Departure Delays, in minutes", y = "Count")
gf_histogram(~ dep_delay, data = nycflights, binwidth = 150) %>%
  gf_labs(x = "Departure Delays, in minutes", y = "Count")
```
...
gf_histogram(~ dep_delay, data = lax_flights) %>%
  gf_labs(x = "Departure Delays, in minutes", y = "Count")
```
...
```{r lax-flights-summ}
favstats(~ dep_delay, data = lax_flights)
```

The `favstats()` function in the `mosaic` package makes several summary statistic functions
for the variable specified, in this case `dep_delay`. We can see the median, the mean, the standard 
deviation (sd), and the sample size (n).
...
above command by specifying the `groups` argument within the `favstats()` function
...
## XX NH 
## Error in Error in UseMethod("arrange_") : no applicable method for 'arrange_' applied to an object of class "c('double', 'numeric')"

# mean(~ dep_delay, groups = month, data = nycflights) %>% 
#  arrange(desc())
...
```{r ot-dep-rate}
## XX BL fix after NH helps with the mean() %>% arrange() case above
...
gf_histogram(~ origin, fill = ~ dep_type, data = nycflights, stat = "count") %>%
  gf_labs(x = "Origin airport", y = "Count")
```
...
gf_histogram(arr_delay ~ dep_delay, color = ~ carrier, data = dl_aa_ua)
```
