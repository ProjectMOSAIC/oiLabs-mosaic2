...
Here we are literally adding a layer on top of our plot. `gf_lm()` creates
the line by fitting a linear model. We can also use the `gf_smooth()` to show the standard error 
associated with our line, but we won't use that for now.
...
Notice here that our model object `m1` can also serve as a data set because stored within it are the fitted values ($\hat{y}$) and the residuals.
