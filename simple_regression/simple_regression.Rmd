...
Let's load up the data for the 2011 season and get a general sense of what we are 
working with:
...
relationship with the correlation coefficient. With the code below, we can
use `gf_point()` to create a scatterplot to visualize a general relationship 
between the number of runs and the number of at bats, while
the `cor()` function calculates the correlation between these two variables. 

```{r cor, message=FALSE}
gf_point(runs ~ at_bats, data = mlb11)
...
\[
  \hat{y} = -2789.2429 + 0.6305 \cdot atbats
\]
...
Here we are literally adding a layer on top of our plot. The `gf_lm()` function 
...
```{r residuals}
gf_point(.stdresid ~ .fitted, data = m1) %>%
  gf_hline(yintercept = 0, linetype = "dashed") %>%
  gf_labs(x = "Fitted Values", y = "Residuals") 
```

Notice here that our model object `m1` can also serve as a data set because stored within it are the fitted 
values ($\hat{y}$) and the residuals.
...
**Nearly normal residuals**: To check this condition, we can look at a histogram with a Normal distribution
curve overlay:

```{r hist-res}
gf_dhistogram(~ .stdresid, data = m1, bins = 6) %>% 
...
from a lab written by the faculty and TAs of UCLA Statistics. These materials were adapted to functions in the `mosaic` and `ggformula` packages by Nicholas Horton and Bonnie Lin (Amherst College).
</div>
